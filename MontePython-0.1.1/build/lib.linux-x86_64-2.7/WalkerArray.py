...
import math,numpy,pickle,MontePython_cxx,copy
...
class WalkerArray(object):
    ...
    def __warray(self):
        ...
        w = numpy.zeros(self.sizeof_walker*self.size,'f')
        last_i=0
        stride=self.sizeof_walker
        # set 
        for i in numpy.arange(0,len(w),stride):
            index = i+skip_size
            w[index] = 0 # e_l
            index += 1
            w[index] = 0 # wave_func
    ...
    def __init__(self, size, particles, dimension, walkerchunk=None):
        ...
        self.size          = size
        self.arraysize     = size
        if isinstance(walkerchunk,numpy.ndarray):
    ...
    def __setitem__(self,w_nr,walkerchunk):
        ...
        """
        if not isinstance(walkerchunk,numpy.ndarray):
            msg = "walkerchunk must be of type numpy.ndarray"
            raise msg
        if isinstance(w_nr,slice):
            start,stop,step,size = self._slicer(w_nr)
            start *= self.sizeof_walker
            ...
            if w_nr == len(self):
                ...
                if len(self)==self.arraysize:
                    self.walkers = numpy.concatenate((self.walkers,
                                                        walkerchunk))
                    self.arraysize+=1
                else:
                    start = w_nr*self.sizeof_walker
