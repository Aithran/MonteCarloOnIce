## Automatically adapted for numpy.oldnumeric Sep 28, 2006 by -c
...
"""Module pypar.py - Parallel Python using MPI and numpy
...
# Meta data
__version__ = '1.9.2_numpy'
__date__ = '05 June 2007'
__author__ = 'Jon K. Nilsen'
...
def send(x, destination, use_buffer=False, vanilla=False,
    ...
    assert isinstance(destination, types.IntType), errmsg
...
def receive(source, buffer=None, vanilla=False, tag=default_tag,
    ...
    if bypass:
        ...
        assert isinstance(source, types.IntType), errmsg
...
def broadcast(buffer, root, vanilla=False):
    ...
    assert isinstance(root, types.IntType), errmsg
...
def create_control_info(x, vanilla=0, return_object=False):
    ...
    if not vanilla:
        if type(x) == types.StringType:
            ...
            typecode = 'c'
            size = len(x)
        elif type(x).__name__ == 'ndarray': #Numeric isn't imported yet
            try:
                import numpy.oldnumeric as Numeric
            except:
...
if error:
    ...
    if rank() == 0:     
        print "Pypar initialised MPI OK with %d processors" %(size())
