...
import re,math,numpy,pickle,os,WalkerArray,copy,time
...
do Jackson-Feenberg
set params          = [0.5, 0, 0, 0, 200, 1, 1, 0]
...
"""
    ...
    def __set_w_block(self,buffer=None):
        ...
        #if buffer is a numpy array, this should be a walker array
    ...
    def __set_work_data(self):
        ...
        self.observables = numpy.zeros((self.steps,3),'d') 
...
######################user functions:########################
    ...
    def all_reduce(self,x):
        xs = self.pypar.gather(numpy.array([x]),self.master_rank)
        return self.pypar.broadcast(float(numpy.sum(xs)),self.master_rank)


    def metropolis_steps(self):
        """Wrap around MCpp().MetropolisStep()"""
        conv = Convert_QickArray()
        counters = numpy.array([self.trials, self.accept])
    ...
    def numpyFloat2IntList(self,x):
        return numpy.array(x.tolist(),'i').tolist()
    ...
    def __find_opt_w_p_node(self):
        ...
        """
        timings = self.pypar.gather(numpy.array([abs(self.t1-self.t0)]),
                                    self.master_rank)
        ...
        self.loc_walkers = self.numpyFloat2IntList(tmp_loc_walkers)
        ...
        while sum(abs(difference))!=0:
            ...
            if self.myrank==i_max:
                ...
                args = [len(self.w_block)+difference[i_max],
                        self.particles,
                        self.dimensions,
                        numpy.concatenate((self.w_block[:],recv_buff))]
                self.w_block = WalkerArray.WalkerArray(*args)
